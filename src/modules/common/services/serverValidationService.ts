...
export class ServerValidationService {
	...
	getAllFormErrors(formName) {
		...
		if (this.hasErrorsForForm(formName)) {
			...
			Object.keys(this.errs[formName]).forEach((key) => {
				if (angular.isArray(this.errs[formName][key])) {
					for (var i = 0; i < this.errs[formName][key].length; i++) {
						allErrs.push(this.errs[formName][key][i]);
					}
				}
			});
