...
function getOptions() {
    ...
    options.moduleStylesPath = './src/modules/'.concat(options.module, '/', options.module, '.scss');
...
/**
 * Build and return the sass import string to inject into the module styles file.
 *
 * @param  {Object} options - The task options.
 * @returns {string} The sass import string to inject into the module styles file.
 */
function getStylingImportString(options) {
    return String.prototype.concat('@import \'src/modules/', options.module, '/components/', options.name, '/', options.name, '\'');
}
...
/**
 * Find the last import in the module styles file then inject import statement
 * of the new component's styles file into the file content and return the result.
 *
 * @param  {string} content - The file content.
 * @param  {Object} options - The task options.
 * @returns {string} The (possibly edited) file content.
 */
function addStylingImport(content, options) {
    const lineIndex = content.lastIndexOf('@import ');
    if (lineIndex > -1) {
        const sliceIndex = content.indexOf(';', lineIndex);
        const importStr = getStylingImportString(options);

        return String.prototype.concat(content.slice(0, sliceIndex + 1), '\r\n', importStr, content.slice(sliceIndex, content.length));
    }
    console.error(String.prototype.concat('Unable to inject the ', options.name, ' style import into the ', options.module, ' module styles file.'));
    return content;
}
...
gulp.task('createComponent', () => {
    ...
    // Create the ts file
    gulp.src('./templates/component.ts.tmpl')
    ...
    gulp.src('./templates/styles.scss.tmpl')
        .pipe(template(options))
        .pipe(rename(options.name.concat('.scss')))
        .pipe(gulp.dest(options.targetLocation));

    /**
     * Edit the module file, injecting the import and definition of the  new
     * component.
     *
     * @param  {string} content - The file content.
    ...
    /**
     * Edit the module styles file, injecting the sass import for the component
     * styles.
     *
     * @param  {string} content - The file content.
     * @param  {Function} done - Async callback function.
     */
    function editModuleStyles(content, done) {
        content = addStylingImport(content, options);
        done(null, content);
    }
    ...
    // Add component styles to module styles.
    // The gulp.dest overwrite option doesn't actually work. Use base.
    // See: https://github.com/gulpjs/gulp/issues/267#issuecomment-199056805
    gulp.src(options.moduleStylesPath, { base: './'})
        .pipe(change(editModuleStyles))
        .pipe(gulp.dest('./'));
